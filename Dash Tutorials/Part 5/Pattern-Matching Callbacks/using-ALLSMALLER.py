from dash import Dash, html, dcc, Input, Output, State, MATCH, ALLSMALLER
import pandas as pd
...
def display_output(matching_value, previous_values):
    ...
    if len(all_values) ==1:
        return html.Div('{:.2f} is the life expectancy of {}'.format(avgLifeExp, matching_value))
    elif len(all_values) == 2:
        return html.Div('{:.2f} is the life expectancy of {}'.format(avgLifeExp, ' and '.join(all_values)))
    else:
        return html.Div('{:.2f} is the life expectancy of {}, and {}'.format(avgLifeExp, ', '.join(all_values[:-1]), all_values[-1]))
